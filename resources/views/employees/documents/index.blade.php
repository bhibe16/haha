        ...
        @foreach($employee->documents as $document)
            <div class="bg-white shadow-md rounded-lg p-4">
                <div class="flex items-center justify-between">
                    <div>
                        <a href="{{ Storage::url($document->file_path) }}" class="text-blue-500 font-semibold" target="_blank">{{ $document->name }}</a>
                        <span class="text-gray-500 block text-sm">Uploaded on {{ $document->created_at->format('d-m-Y') }}</span>
                    </div>
                    <form action="{{ route('employee.document.delete', ['employee' => $employee->id, 'document' => $document->id]) }}" method="POST">
                        @csrf
                        @method('DELETE')
